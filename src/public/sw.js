  ...
  event.respondWith(
    caches.match(event.request).then((response) => {
      if (response) {
        ...
        } else if (event.request.url.includes('.webmanifest') && response.headers.get('Content-Type') !== 'application/manifest+json') { // Corrected condition for webmanifest
            console.warn('Correcting Content-Type for webmanifest from cache!');
            const headers = new Headers(response.headers);
